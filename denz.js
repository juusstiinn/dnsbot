  ...
  break
			...
			if (!isUser) return reply(mess.only.userB)
				...
				case 'daftar':
					...
					captionnya = `╭─「 *_𝙸𝙽𝙳𝙴𝙽𝚃𝙸𝙵𝙸𝙲𝙰𝙲𝙸𝙾𝙽 𝙳𝙴 𝚄𝙽 𝙼𝙼𝙶_* 」\`\`\`\n│ 𝚁𝙰𝙿𝙰 𝚃𝚄 𝙼𝙰𝙳𝚁𝙴: \n│TM08GK8PPHBSJDH10J\`\`\`\n│\n│\`\`\`ᴘᴀᴅᴀ ${date} ${time}\`\`\`\n│\`\`\`「 𝙽𝙾𝙼𝙱𝚁𝙴 」: ${pushname2}\`\`\`\n│\`\`\`「 𝙽𝚄𝙼𝙴𝚁𝙾」: wa.me/${sender.split("@")[0]}\`\`\`\n│\`\`\`𝚈𝙰 𝙿𝚄𝙴𝙳𝙴𝚂 𝙿𝙾𝙽𝙴𝚁\`\`\`\n│\`\`\`𝙼𝙴𝙽𝚄\`\`\`\n│\`\`\`𝚂𝙸𝙽𝙶𝙰 𝚃𝚄 𝙼𝙰𝙳𝚁𝙴 ${prefix}menu\`\`\`\n│\`\`\`\n│𝙴𝚂 𝚄𝙽 𝙼𝙼𝙶: ${user.length} 𝙲𝙾𝙽𝙵𝙸𝚁𝙼𝙰𝙳𝙾\`\`\`\n╰────────────────`
					brando = await getBuffer(`https://pencarikode.xyz/api/cita-cita?apikey=pais`, {method: 'get'})                
...
// only grup fitur anime
              case 'anime':
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					if (!isAnime) return reply(' *𝚜𝚊𝚋𝚎𝚜 𝚊𝚗𝚝𝚎𝚜 𝚜𝚘𝚕𝚒𝚊 𝚜𝚎𝚛 𝚌𝚘𝚖𝚘 𝚝𝚞* ')
					reply(mess.wait)
  ...
  break
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					if (mentioned.length > 1) {
						...
						mentions(`𝙳𝙴𝙿𝚄𝚁𝙰𝙳𝙾 𝙿𝙾𝚁 𝙼𝙼𝙶   : @${mentioned[0].split('@')[0]}`, mentioned, true)
						denz.groupRemove(from, mentioned)
					}
					break 
				case 'kicktime':
...
case 'husbu':
        ...
        if (budy.includes("Parari")){
		denz.updatePresence(from, Presence.composing)
		const loli = fs.readFileSync('./mp3/Parari.mp3')
        denz.sendMessage(from, loli, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
        const d = fs.readFileSync('./sticker/Parari.webp');
        ...
        if (budy.includes("Mujeres")){
		denz.updatePresence(from, Presence.composing)
		const loli = fs.readFileSync('./mp3/Mujeres.m4a')
        denz.sendMessage(from, loli, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
        const d = fs.readFileSync('./sticker/Mujeres.webp');
        denz.sendMessage(from, d, sticker, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "song : dj tapi boong", 'jpegThumbnail': fs.readFileSync('./sticker/dnsnew.webp')}}}})
        }
